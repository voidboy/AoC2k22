...
fn fix_up_down(knots: &mut Vec<Pos>, visit: &mut HashSet<Pos>) {
...
fn fix_left_right(knots: &mut Vec<Pos>, visit: &mut HashSet<Pos>) {
    for i in 1..knots.len() {
    ...
    for line in contents.lines() {
        ...
        match direction {
            "U" => {
                for _ in 0..length {
                    knots[0].1 += 1;
                    fix_up_down(&mut knots, &mut visit);
                }
            }
            "D" => {
                for _ in 0..length {
            ...
            "R" => {
                for _ in 0..length {
                    knots[0].0 += 1;
                    fix_left_right(&mut knots, &mut visit);
                }
            }
            "L" => {
                for _ in 0..length {
